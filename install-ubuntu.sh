...
software="nginx apache2 apache2-utils apache2-suexec-custom bsdutils e2fsprogs
    ...
    php5-curl imagemagick php5-imagick libapache2-mod-php5 mc exim4 exim4-daemon-heavy
    clamav-daemon flex dovecot-imapd dovecot-pop3d vsftpd phpMyAdmin awstats e2fslibs
...
if [ -z $email ]; then
    ...
    if [ -z "$servername" ]; then
        read -p "Please enter hostname [$(hostname -f)]: " servername
    fi
fi
